...
const enabledClaudeSettings: ClaudeSettings = Schema.decodeSync(ClaudeSettings)({
  enabled: true,
});
const enabledCodexSettings: CodexSettings = Schema.decodeSync(CodexSettings)({
  enabled: true,
});
...
process.env.MEER_CODE_CURSOR_ENABLED = "1";
  ...
  (it) => {
    describe("checkCodexProviderStatus", () => {
      it.effect("uses the app-server account and model list for provider status", () =>
        Effect.gen(function* () {
          const status = yield* checkCodexProviderStatus(enabledCodexSettings, () =>
            Effect.succeed(
      ...
      it.effect("closes the app-server probe scope when provider status times out", () =>
        Effect.gen(function* () {
          ...
          const statusFiber = yield* checkCodexProviderStatus(enabledCodexSettings).pipe(
    ...
    describe("ProviderRegistryLive", () => {
      ...
      // probe turns it into the user-visible "not installed" error snapshot.
      // If the aggregator's `syncLiveSources` breaks, that snapshot never
      // lands in `getProviders` and the assertions below fail.
      it.effect("propagates real Meer probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          const missingBinary = `/tmp/meer_code_missing_`;
            ...
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                providers: {
                  ...
                  meer: { enabled: true, binaryPath: missingBinary },
                },
              }),
            ),
          );
          ...
          yield* Effect.gen(function* () {
            ...
            const meerProvider = providers.find((provider) => provider.instanceId === "meer");
            assert.notStrictEqual(
              meerProvider,
              undefined,
              `Expected the aggregator to know about meer; instead saw: ${providers
            ...
            assert.strictEqual(
              meerProvider?.status,
              "error",
              "Real Meer probe against a missing binary should surface as 'error' in the aggregator",
            );
            assert.strictEqual(meerProvider?.installed, false);
            assert.strictEqual(
              meerProvider?.message,
              "Meer CLI (`meer`) is not installed or not on PATH.",
            );
          }).pipe(Effect.provide(runtimeServices));
        }),
      );

      // Guards the settings hot-reload path: changing
      // `providers.meer.binaryPath` in settings must tear down the live
      ...
      //
      it.effect("re-probes when settings change the Meer binaryPath", () =>
        Effect.gen(function* () {
          const firstMissing = `/tmp/meer_code_first_`;
          const secondMissing = `/tmp/meer_code_second_`;
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
                providers: {
                  ...
                  cursor: { enabled: false },
                  opencode: { enabled: false },
                  meer: { enabled: true, binaryPath: firstMissing },
                },
              }),
            ),
          );
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry;
            // Boot-time probe: the default meer instance is enabled with
            ...
            const initialMeer = initialProviders.find((provider) => provider.instanceId === "meer");
            assert.strictEqual(initialMeer?.status, "error");
            assert.strictEqual(initialMeer?.installed, false);
            const initialCheckedAt = initialMeer?.checkedAt;
            ...
            yield* serverSettings.updateSettings({
              providers: {
                meer: { enabled: true, binaryPath: secondMissing },
              },
            });
            ...
            const refreshed = yield* Effect.gen(function* () {
              for (let attempts = 0; attempts < 60; attempts += 1) {
                const providers = yield* registry.getProviders;
                const meer = providers.find((provider) => provider.instanceId === "meer");
                if (meer !== undefined && meer.checkedAt !== initialCheckedAt) {
                  return providers;
                }
            ...
            const reprobedMeer = refreshed.find((provider) => provider.instanceId === "meer");
            assert.notStrictEqual(
              reprobedMeer?.checkedAt,
              initialCheckedAt,
              "Expected a fresh probe after settings change, got the stale snapshot",
            );
            assert.strictEqual(reprobedMeer?.status, "error");
            assert.strictEqual(reprobedMeer?.installed, false);
          }).pipe(Effect.provide(runtimeServices));
        }),
      );
      ...
      it.effect("keeps Meer disabled and skips probing when the provider setting is disabled", () =>
        Effect.gen(function* () {
          const serverSettings = yield* makeMutableServerSettingsService(
            decodeServerSettings(
              deepMerge(encodedDefaultServerSettings, {
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry;
            const providers = yield* registry.getProviders;
            const meerProvider = providers.find(
              (provider) => provider.instanceId === ProviderInstanceId.make("meer"),
            );
            ...
            assert.strictEqual(meerProvider?.enabled, false);
            assert.strictEqual(meerProvider?.status, "disabled");
            assert.strictEqual(meerProvider?.message, "Meer is disabled in Meer Code settings.");
            assert.strictEqual(meerSpawned, false);
          }).pipe(Effect.provide(runtimeServices));
        }),
      );
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in Meer Code settings.");
        }),
      );
    });
