      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            ...
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"meerCodeCommitHash":"abcdef1234567890"}'),
          }),
        ),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Meer Code"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Meer Code");
