...
const AppPackageMetadata = Schema.Struct({
  meerCodeCommitHash: Schema.optional(Schema.String),
...
>()("@meer-ai/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.meerCodeCommitHash ?? parsed.t3codeCommitHash).pipe(
