...
import { ClientSettingsSchema, type ClientSettings } from "@meer-ai/contracts";
...
function makeLayer(baseDir: string) {
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ MEER_CODE_HOME: baseDir })),
    ),
  );
