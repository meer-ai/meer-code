...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(
        page.getByText("Meer Code will restart to expose this environment over the network."),
      )
      .toBeInTheDocument();
  ...
  it("shows the Meer binary path field in provider settings", async () => {
    setServerConfigSnapshot(createBaseServerConfig());
    ...
    await page.getByLabelText("Toggle Meer details").click();

    await expect.element(page.getByText("Binary path")).toBeInTheDocument();
    await expect.element(page.getByLabelText("Binary path")).toHaveValue("meer");
  });
  ...
  it("runs one-click provider updates from the provider card", async () => {
    const updateProvider = vi.fn<LocalApi["server"]["updateProvider"]>().mockResolvedValue({
      providers: [createOutdatedProvider("meer", "npm install -g meerai@latest")],
    });
    window.nativeApi = {
      persistence: {
        getClientSettings: vi.fn().mockResolvedValue(null),
    ...
    expect(updateProvider).toHaveBeenCalledWith({
      provider: ProviderDriverKind.make("meer"),
  ...
  it("keeps long provider update commands inside the fixed-width popover", async () => {
    ...
    setServerConfigSnapshot({
      ...createBaseServerConfig(),
      providers: [createOutdatedProvider("meer", longUpdateCommand)],
    });
