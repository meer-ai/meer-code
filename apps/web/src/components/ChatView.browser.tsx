      ...
      await vi.waitFor(
        () => {
          ...
          expect(attachRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              MEER_CODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(attachRequest?.env?.MEER_CODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              MEER_CODE_PROJECT_ROOT: "/repo/project",
              MEER_CODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: Meer Code Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
