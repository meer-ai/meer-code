...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@meer-ai/ssh/auth/SshPasswordPrompt",
) {
...
# Invoked by ssh via SSH_ASKPASS when Meer Code re-runs ssh with a cached password
...
printf 'Meer Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Meer Code re-runs\r
...
[Console]::Error.WriteLine("Meer Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
