...
export function createSourceControlDiscoveryManager<TKey extends string = string>(
    ...
    const promise = resolvedClient.discoverSourceControl().then(
      (result) => {
        if (getRefreshVersion(targetKey) === refreshVersion) {
          lastSuccessfulRefreshAt.set(targetKey, Effect.runSync(Clock.currentTimeMillis));
  ...
  function invalidate(target?: SourceControlDiscoveryTarget<TKey>): void {
    ...
    refreshInFlight.delete(targetKey);
    lastSuccessfulRefreshAt.delete(targetKey);
  ...
  function watch(
    ...
    if (client) {
      ...
      const sync = () => {
        ...
        if (!isClientReplacement && isFreshSnapshot(targetKey)) {
          return;
        }
  ...
  function isFreshSnapshot(targetKey: string): boolean {
    const refreshedAt = lastSuccessfulRefreshAt.get(targetKey);
    if (refreshedAt === undefined) {
      return false;
    }
    const current = config.getRegistry().get(sourceControlDiscoveryStateAtom(targetKey));
    return (
      current.data !== null && Effect.runSync(Clock.currentTimeMillis) - refreshedAt < staleTimeMs
    );
  }
