...
export const DEFAULT_MEER_CODE_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".meer-code"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MEER_CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("MEER_CODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid MEER_CODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `MEER_CODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric MEER_CODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed MEER_CODE_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_MEER_CODE_HOME;
  });
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      MEER_CODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.MEER_CODE_PORT = String(serverPort);
      ...
      delete output.MEER_CODE_MODE;
      delete output.MEER_CODE_NO_BROWSER;
      delete output.MEER_CODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.MEER_CODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.MEER_CODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.MEER_CODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MEER_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.MEER_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MEER_CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MEER_CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.MEER_CODE_MODE = "web";
      delete output.MEER_CODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read MEER_CODE_PORT_OFFSET/MEER_CODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MEER_CODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.MEER_CODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Meer Code data (equivalent to MEER_CODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("MEER_CODE_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MEER_CODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MEER_CODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MEER_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("MEER_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to MEER_CODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MEER_CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MEER_CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MEER_CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MEER_CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MEER_CODE_PORT")),
  ),
