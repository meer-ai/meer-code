...
- Publishes the CLI package (`apps/server`, npm package `meer-code`) with OIDC trusted publishing from the same workflow file:
...
- `MEER_CODE_WEB_ROUTER_URL`: defaults to `https://code.meerai.dev`.
- `MEER_CODE_WEB_LATEST_DOMAIN`: defaults to `latest.code.meerai.dev`.
- `MEER_CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.code.meerai.dev`.

Runtime and release configuration now uses `MEER_CODE_*` environment variables.
The server, desktop app, and release scripts still accept legacy `T3CODE_*`
variables as aliases when the matching `MEER_CODE_*` variable is not set.

Required Vercel domains:

- `code.meerai.dev`: the router domain users open, updated by stable releases.
- `latest.code.meerai.dev`: channel alias updated by stable releases.
- `nightly.code.meerai.dev`: channel alias updated by nightly releases.

These domains need DNS records pointing to Vercel and must be added to the
Vercel project before hosted pairing or hosted channel selection can work for
public users.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__meer-code/channel?channel=latest` or
`/__meer-code/channel?channel=nightly`; the router stores the
`meer_code_web_channel` cookie and rewrites future requests on `code.meerai.dev` to
...
through `/__meer-code/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `code.meerai.dev` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `meer-code`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `MEER_CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `MEER_CODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `meer-code` (or rename package first if needed).
...
Unsigned artifacts are useful for internal validation only:

- macOS users will see Gatekeeper warnings and may need to bypass quarantine.
- Windows users will see SmartScreen warnings.
- Linux AppImage artifacts can be tested normally after marking them executable.

Do not treat an unsigned dry run as a polished public launch.
