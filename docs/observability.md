...
Meer Code has one server-side observability model:
...
```bash
export MEER_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export MEER_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export MEER_CODE_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export MEER_CODE_TRACE_MIN_LEVEL=Info
export MEER_CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `MEER_CODE_OTLP_*`.

macOS app bundle example:

```bash
MEER_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
MEER_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
MEER_CODE_OTLP_SERVICE_NAME=t3-desktop \
"/Applications/Meer Code.app/Contents/MacOS/Meer Code"
```
...
```bash
tail -f "$MEER_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$MEER_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$MEER_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- `MEER_CODE_OTLP_TRACES_URL` was not set
...
- `MEER_CODE_TRACE_FILE`: override trace file path
- `MEER_CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `MEER_CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `MEER_CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `MEER_CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `MEER_CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `MEER_CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `MEER_CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `MEER_CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `MEER_CODE_OTLP_SERVICE_NAME`: service name, default `t3-server`
